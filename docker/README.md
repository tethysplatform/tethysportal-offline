...
```
> docker rm tethysportal-offline
> docker rmi tethysportal-offline
```

2. Build a new docker with the desired name and tag
```
> docker build -t tethysportal-offline:latest
```
...
```
> docker build [--build-arg ARG=VAL] -t tethysportal-offline:latest
```
...
Example of Command:
```
> docker run -p 127.0.0.1:8000:8000 --name tethysportal-offline \
    ...
    -e TETHYSBUILD_SUPERUSER_PASS='admin' tethysportal-offline
```
